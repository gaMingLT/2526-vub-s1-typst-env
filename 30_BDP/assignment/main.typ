...
This section will discuss the implementation (code) for the project. Full project code can be found in the associated Apache Spark project, or small snippets will be placed throughout the text.
...
The loader or `TrafficLoader.scala` file is responsible for loading the correct set of data files. For this, 3 different methods are created for each type of file: `loadVolume`, `loadSpeed` and `loadFeatures`. Loading the correct file is done based on the `dataset` value which is an enum, as shown in listing
...
This step of the pipeline will apply the Vector Assembler to the selected columns, to create a `features` column for the given dataframe. This is done once for the historical training data and multiple times during the prediction phase for the test data.
...
Furthermore, the combination of each node and the list of static features is selected from the `transformedDF` dataframe to which the `distinct` operation is applied, creating the unique combination of each node with it static features (`nodeFeaturesDF`).

To further reduce the number of rows required to be kept in memory during the prediction phase, a select number of rows, 287 to be specific, is selected from the `transformedDF` by the `takeHistory` method. The number of rows is specific to the maximum number of rows required for the time-series step. One of which is shifting the speed rows by 1 day (288 rows).
...
During iteration, the additional rows are created by adding the `timestamp`, `speed` and `volume` columns to the `nodeFeaturesDF`. The future rows are added to the historical dataframe (`dataDF`) by using `unionByName`.  The time-series columns are created by applying the `addTimeSeries` method on the dataframe. Once the time-series columns have been added, the features columns are generated.
...
Starting with the `M_1000` dataset, increasing the number of partitions has an impact on the time taken, but not much. It could be said, that for the `M_5000` dataset, this trend would propagate itself.
...
During testing, the `df.cache()` was placed in several places to measure the impact on the performance, but it was concluded that for the current implementation there was a negative impact on the performance. Therefore, there was no use of cache or persist throughout the implementation.
